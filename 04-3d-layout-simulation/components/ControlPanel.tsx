...
const ControlPanel: React.FC<ControlPanelProps> = ({
  ...
  const ratios: { value: AspectRatio; label: string }[] = [
    ...
    { value: "4:3", label: "4:3 (標準相片)" },
    { value: "3:4", label: "3:4 (直式相片)" },
    ...
    <div className="w-full md:w-80 bg-slate-800 border-l border-slate-700 flex flex-col h-full overflow-y-auto scrollbar-thin scrollbar-thumb-slate-600">
      <div className="p-6 space-y-8">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm">設定您的視角與風格。</p>
        </div>

        {/* Camera Control Section */}
        <div className="space-y-6 border-b border-slate-700 pb-6">
            <h3 className="text-sm font-semibold text-slate-200 flex items-center gap-2">
                <Eye className="w-4 h-4" /> 相機視角
            </h3>
            ...
            <div className="space-y-3">
             ...
             <p className="text-xs text-slate-500 mt-2">
                {!hasMarker ? "請先在圖面上點擊以設定位置。" : "調整參數以改變相機位置。"}
            </p>
        </div>
        ...
        <div className="space-y-6">
            ...
            <div className="relative">
                ...
                className="w-full bg-slate-900 border border-slate-700 rounded-lg py-3 pl-3 pr-10 text-sm text-slate-200 focus:ring-2 focus:ring-blue-500 focus:outline-none appearance-none cursor-pointer hover:bg-slate-800 transition-colors"
                >
                {styles.map((style) => (
            ...
            {/* Ratio Selection */}
            <div className="space-y-3">
                <label className="text-xs font-medium text-slate-300">圖片比例</label>
                <div className="relative">
                    <select
                    value={config.ratio}
        ...
        <div className="space-y-3 pt-4 border-t border-slate-700">
          <label className="text-sm font-medium text-slate-300">文字說明加強 (選填)</label>
          <textarea
            value={config.prompt}
            onChange={(e) => onConfigChange({ ...config, prompt: e.target.value })}
            placeholder="例如：明亮的陽光、木質地板、挑高天花板..."
            className="w-full h-24 bg-slate-900 border border-slate-700 rounded-lg p-3 text-sm text-slate-200 placeholder-slate-500 focus:ring-2 focus:ring-blue-500 focus:outline-none resize-none"
          />
        </div>
        ...
        <div className="space-y-3 pt-4 border-t border-slate-700 pb-10">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 px-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg text-white font-semibold shadow-lg transition-all"
          >
            {isGenerating ? (
              <>
