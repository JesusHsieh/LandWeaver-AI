...
const Step4Presentation: React.FC<Step4PresentationProps> = ({ finalPlanImage, generatedScenes, style, language }) => {
    ...
    const renderLoading = (messageKey: 'generatingPresentation' | 'generatingSlides') => (
        <div className="flex flex-col items-center justify-center h-64">
            <div className="loader border-4 border-emerald-200 border-t-emerald-700 rounded-full w-12 h-12 animate-spin mb-4" />
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    {themes.map(theme => (
                        <button 
                            ...
                            className={`p-4 rounded-lg border-2 transition-all duration-200 ${selectedTheme.nameKey === theme.nameKey ? 'border-emerald-600 ring-2 ring-emerald-600' : 'border-slate-300 hover:border-emerald-500'}`}
                        >
