...
const Step3SceneGeneration: React.FC<Step3SceneGenerationProps> = ({ 
    ...
    useEffect(() => {
        const fetchStyles = async () => {
            ...
            try {
                // Pass language to get suggestions in the correct language
                const styles = await suggestStyleIdeas(language);
    ...
    const downloadAllScenesAsZip = async () => {
        ...
        try {
            ...
            link.download = `landscape_scenes_${new Date().toISOString().slice(0,10)}.zip`;
                ...
                <div className="flex flex-wrap items-center justify-center gap-4 w-full">
                    <div className="flex-grow flex max-w-md">
                        <input 
                            ...
                            className="flex-grow px-4 py-2 border border-slate-300 rounded-l-lg focus:ring-2 focus:ring-emerald-500 bg-white text-slate-900"
                        ...
                        <button
                            ...
                            className="px-4 py-2 bg-emerald-600 text-white font-semibold rounded-r-lg hover:bg-emerald-700 transition-colors disabled:opacity-50 flex items-center whitespace-nowrap"
                        >
                            {isSuggestingStyle ? (
                                <>
                            ...
                            {suggestedStyles.map((styleName, index) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`rounded-lg border-2 text-center p-2 h-24 transition-all duration-200 group focus:outline-none flex flex-col items-center justify-center gap-1 ${isSelected ? 'border-emerald-600 ring-2 ring-emerald-500 bg-emerald-600 text-white' : 'border-slate-300 bg-white hover:border-emerald-400 hover:bg-emerald-50 text-slate-800'}`}
                                    >
                                        <span className="text-3xl" role="img" aria-label="style icon">{styleEmojis[index % styleEmojis.length]}</span>
                                        <span className="text-xs font-bold leading-tight line-clamp-2">{styleName}</span>
                                    </button>
                                );
                            })}
                        </div>
                    ...
                    if (scene?.error) {
                        return (
                            <div key={index} className="bg-red-100 aspect-square rounded-lg flex flex-col items-center justify-center text-red-600 p-4">
                                ...
                                <button 
                                    onClick={generateScenes}
                                    className="mt-2 px-3 py-1 bg-emerald-500 text-white text-xs rounded hover:bg-emerald-600"
                                >
